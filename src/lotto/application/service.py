...
class LottoService:
    def __init__(
        self,
        lotto_repository: ILottoRepository,
        user_repository: IUserRepository,
    ):
        self.lotto_repository = lotto_repository
        self.user_repository = user_repository

    async def fetch_lotto_data_from_api(
        self, drw_no: int
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"회차 {drw_no}: HTTP 오류 - {response.status_code}"
                    )
                    return None
    ...
    async def update_next_lotto_draw(self) -> bool:
        ...
        try:
            ...
            # 2. 이미 존재하는지 확인
            existing_draw = await self.lotto_repository.get_lotto_draw_by_round(
                next_round
            )
            if existing_draw:
            ...
            if not api_data:
                logger.warning(
                    f"회차 {next_round}: API에서 데이터를 가져올 수 없음"
                )
                return False
    ...
    async def create_lotto_recommendation(
        ...
        # 3. 통계 데이터 조회
        frequent_nums = await self.lotto_repository.get_frequent_numbers(
            limit=10
        )
        infrequent_nums = await self.lotto_repository.get_excluded_numbers(
            limit=2
        )
        ...
        # 6. 데이터베이스에 저장
        recommendation = (
            await self.lotto_repository.create_lotto_recommendation(
                user_id=user_id,
    ...
    async def get_lotto_recommendation(
        ...
        if recommendation:
            ...
            # 추천 결과를 확인하지 않은 경우, 해당 회차 응답
                ...
                if latest_round >= _round:
                    is_finished = True

                content = LottoRecommendationContent.model_validate(
    ...
    def _extract_rec_numbers(self, rec_content) -> List[int]:
        ...
        getter = (
            (lambda k: getattr(rec_content, k))
            if hasattr(rec_content, "__getattribute__")
            and not isinstance(rec_content, dict)
            else (lambda k: rec_content[k])
        )
        return [
            int(getter(k))
    ...
    def _pick_prize_amount(self, draw, rank: Optional[int]) -> Optional[int]:
        ...
        rank_map = {
            2: "second_prize_amount",
            3: "third_prize_amount",
            4: "fourth_prize_amount",
            5: "fifth_prize_amount",
        }
        return getattr(draw, rank_map.get(rank, ""), None)

    async def check_recommendation_result(
        self, user_id: str, round: int
    ) -> LottoResultCheckResponse:
        # 1) 해당 회차 추천 레코드
        rec = await self.lotto_repository.get_recommendation_by_user_and_round(
            user_id=user_id, round=round
        )
        if not rec:
            raise HTTPException(
                status_code=404, detail="해당 회차의 추천 기록이 없습니다."
            )
        if not rec.content:
            raise HTTPException(
                status_code=400, detail="추천 내용이 비어 있습니다."
            )
        ...
        if not draw:
            raise HTTPException(
                status_code=404,
                detail="해당 회차의 당첨 결과가 아직 등록되지 않았습니다.",
            )
        ...
        has_bonus = (bonus_number in rec_set) and (
            bonus_number not in main_set
        )  # 규칙상 보너스는 메인과 중복 안 됨
